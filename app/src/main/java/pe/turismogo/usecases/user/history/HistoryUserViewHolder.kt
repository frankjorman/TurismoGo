...
class HistoryUserViewHolder(view : View) : RecyclerView.ViewHolder(view) {
    ...
    fun bind(event: Event) {
        ...
        val departure = "${context.getString(R.string.prompt_date_departure)}: ${event.departureDate}"
        val returnDate = "${context.getString(R.string.prompt_date_return)}: ${event.returnDate}"

        tvEventName.text = name
        tvEventDescription.text = description

        tvEventDates.text = returnDate
        tvEventCost.text = departure

        tvEventRoomAvailable.visibility = View.INVISIBLE
        btnEventDetails.visibility = View.INVISIBLE
    }
